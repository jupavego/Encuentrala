...
// dentro. Devuelve null si no hay ni municipio elegido.
function zonaActiva() {
  // Solo cuenta la zona que el usuario ELIGIO. Corregimiento y vereda tambien
  // se rellenan solos al mover el pin (ver sincronizarTerritorioConPunto), y
  // esos no deben acotar el listado: si solo se filtro el municipio, "todas
  // las UDS" son las del municipio entero.
  if (TERRITORIO && TERRITORIO_ELEGIDO) {
    const z = TERRITORIO.tipo === "corr" ? V.corregimientos[TERRITORIO.idx] : V.veredas[TERRITORIO.idx];
    const etiq = TERRITORIO.tipo === "corr" ? "el corregimiento"
      : ((z.t && z.t !== "VE") ? "la zona" : "la vereda");
    return { tipo: TERRITORIO.tipo, idx: TERRITORIO.idx, nombre: z.n, etiqueta: etiq };
  }
  const im = idxMuniActual();
  if (im < 0) return null;
  return { tipo: "muni", idx: im, nombre: V.municipios[im], etiqueta: "el municipio de" };
}
...
// Distingue una zona ELEGIDA por el usuario de una que solo se autocompleto
// con el pin. Importa para el modo por zona: al elegir un municipio, el
// sitio rellena solo corregimiento y vereda segun donde cayo el pin, asi
// que sin esta bandera "ver todas las UDS de Ituango" listaba unicamente
// las de la vereda donde estaba el pin (7 en vez de 38).
let TERRITORIO_ELEGIDO = false;
...
// Modo de consulta. false (por defecto) = "por cercania": se dibujan los
// anillos y se listan las UDS dentro del radio adaptativo. true = "por zona":
// no hay radio ni anillos, y se listan TODAS las UDS que caen dentro del
// poligono de lo que este filtrado (vereda, corregimiento o municipio).
// Responde una pregunta distinta -- "que hay en esta zona" en vez de "que hay
// cerca de este punto" -- y por eso cambia tambien como se pintan los
// resultados: sin bandas de distancia, una sola tabla.
let SIN_RADIO = false;
...
function sincronizarTerritorioConPunto() {
  if (!PUNTO) return;
  const im = idxMuniActual();
  if (im < 0) { TERRITORIO = null; TERRITORIO_ELEGIDO = false; pintarCamposTerritorio(); return; }
  ...
  // si es la MISMA vereda que ya estaba, no se toca la bandera: al elegir una
  // vereda el pin salta a su centro y esta funcion la vuelve a detectar --
  // sin esta condicion, esa deteccion marcaba como "autocompletada" la vereda
  // que el usuario acababa de elegir, y el modo por zona se iba al municipio
  // entero (38 UDS en vez de las 2 de la vereda).
  const misma = antes && antes.tipo === "vereda" && antes.idx === encontrada;
  TERRITORIO = { tipo: "vereda", idx: encontrada };
  if (!misma) TERRITORIO_ELEGIDO = false;   // la detecto el pin, no la pidio el usuario
...
// El rotulo nombra la zona concreta que se va a listar ("Ver todas las UDS de
// Santa Rita") en vez de un generico: lo que hace el boton cambia segun si
// hay vereda, corregimiento o solo municipio filtrado, y asi se sabe de
// antemano que va a pasar al pulsarlo.
function actualizarBotonModo() {
  const b = $("#btnModoZona");
  if (!b) return;
  const z = zonaActiva();
  b.classList.toggle("activo", SIN_RADIO);
  if (SIN_RADIO) {
    b.textContent = "Volver al radio de cercanía";
    b.title = "Vuelve a listar solo las UDS cercanas al pin";
    return;
  }
  b.textContent = z ? "Ver todas las UDS de " + z.nombre : "Ver todas las UDS de la zona";
  b.title = z
    ? "Lista todas las UDS dentro de " + z.etiqueta + " " + z.nombre +
      ", sin importar a que distancia queden del pin"
    : "Lista todas las UDS de la zona filtrada";
}
...
function habilitarFiltrosTerritorio(activo) {
  ...
  $("#btnModoZona").classList.toggle("visible", activo);
  actualizarBotonModo();
}
...
function initMapaBase() {
  ...
  // zoomSnap:0 -> se permiten niveles de zoom FRACCIONARIOS.
  // Por defecto Leaflet solo usa enteros, asi que fitBounds baja al entero
  // inmediatamente inferior al que haria falta: si un municipio no cabe justo
  // en el zoom 9, se muestra en el 8, que abarca el doble de area a lo ancho
  // y a lo alto. Eso hacia que al elegir Ituango se viera media Antioquia con
  // el municipio como una mancha pequena en el centro -- el contorno estaba
  // completo, pero ocupando una fraccion diminuta del mapa.
  // Con snap 0, fitBounds puede quedarse en 8.7 y el contorno llena el
  // recuadro. Afecta a todos los encuadres del sitio (municipio, zona,
  // anillos), siempre para ajustarlos mejor.
  const map = L.map(div, { scrollWheelZoom: true, zoomSnap: 0 });
  MAPA = map;
...
// Pinta en el mapa un marcador por UDS. Se usa igual en los dos modos de
// consulta (por cercania y por zona), asi que vive aparte en vez de estar
// incrustada dentro de buscarYPintar.
...
/* ---------------- modo por zona (sin radio de cercania) ----------------
   Responde "que UDS hay dentro de esta zona", no "cuales quedan cerca del
   pin". Por eso no hay anillos ni bandas de distancia: una sola tabla con
   todo lo que cae dentro del poligono filtrado. La distancia al pin se
   conserva como columna -- sigue siendo util para ordenar -- pero ya no
   decide que entra y que no. */
function pintarModoZona(zona, lista, resWrap, sub, dlWrap, mantenerVista) {
  const cupos = lista.reduce((s, x) => s + (x.p.cu || 0), 0);
  ...
  resWrap.innerHTML = "";
  const aviso = el("p", "nota aviso-modo-zona",
    "Mostrando <b>todas</b> las UDS dentro de " + esc(zona.etiqueta) + " <b>" + esc(zona.nombre) +
    "</b>, sin importar a qué distancia queden del pin. " +
    "El radio de cercanía está desactivado.");
  resWrap.appendChild(aviso);

  if (!lista.length) {
    resWrap.appendChild(el("p", "nota",
      "No hay ninguna UDS georreferenciada dentro de esa zona."));
  } else {
    const cols = [
      ...
      { label: "Distancia al pin", get: x => x.d, fmt: x => fmtDist(x.d) },
    ];
    resWrap.appendChild(tabla(cols, lista, {
      sortCol: 12, sortAsc: true, getId: x => x.p.id, onRowClick: x => abrirPopup(x.p.id),
    }));
  }

  pintarMarcadores(lista);
  // encuadre a la zona completa, no al pin: lo que se esta mirando es el
  // poligono entero.
  if (MAPA && !mantenerVista) {
    const capa = CAPA_ZONA || POLIGONO_MUNI;
    if (capa && capa.getBounds) MAPA.fitBounds(capa.getBounds(), { padding: [10, 10], animate: false });
  }
}
...
function buscarYPintar(mantenerVista) {
  ...
  actualizarBotonModo();
  // MODO POR ZONA: no hay radio ni anillos. Se listan todas las UDS que caen
  // dentro del poligono de lo que este filtrado -- vereda, corregimiento o
  // municipio, lo mas especifico que haya. Se sigue calculando la distancia
  // al pin porque ordenar por ella sigue siendo util, pero ya no filtra.
  const zona = SIN_RADIO ? zonaActiva() : null;
  if (zona) {
    ANILLOS.forEach(a => MAPA && MAPA.removeLayer(a));
    ANILLOS = [];
    const lista = udsDeZona(zona)
      .map(p => ({ p, d: distMetros(PUNTO.lat, PUNTO.lon, p.y, p.x) }))
      .sort((a, b) => a.d - b.d);
    pintarModoZona(zona, lista, resWrap, sub, dlWrap, mantenerVista);
    return;
  }
...
function onMunicipioChange(nombre) {
  ...
  inputDir.value = "";
  $("#estadoDir").textContent = "Se ve " + MUNI.nombre + " completo, con el pin en la cabecera. " +
    "Arrástralo hasta el punto exacto, busca una dirección arriba, o elige una vereda " +
    "(la cabecera está en esa lista) para acercarte a una zona.";
  $("#candidatosDir").innerHTML = "";
  ...
  // Los dos con mantenerVista=true, para que el encuadre al POLIGONO COMPLETO
  // de arriba sea el que quede. Sin eso, colocarPin hacia un setView a zoom
  // >=12 sobre el pin y buscarYPintar reencuadraba al anillo de busqueda: el
  // municipio se abria mostrando solo el casco urbano y sus alrededores, y no
  // se veia la extension real de lo que se acababa de elegir.
  // El pin sigue cayendo en la cabecera -- solo cambia lo que se ENCUADRA --
  // asi que la busqueda por cercania de al lado no se altera. Para mirar solo
  // la cabecera esta el filtro de vereda, donde aparece como una vereda mas
  // (tipo "Cabecera municipal", una por municipio).
  colocarPin(false, true);
  buscarYPintar(true);
}
