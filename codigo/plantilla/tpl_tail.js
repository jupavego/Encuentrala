...
function initMapaBase() {
  ...
  map.fitBounds([[bb[1], bb[0]], [bb[3], bb[2]]], { padding: [4, 4] });
  ...
  // pin por defecto ya puesto desde el arranque (centro geografico del
  // departamento), draggable de una, para quien solo quiere arrastrarlo a
  // su punto sin elegir municipio ni escribir nada.
  PUNTO = { lat: (bb[1] + bb[3]) / 2, lon: (bb[0] + bb[2]) / 2 };
  colocarPin(false);
  buscarYPintar();
}

function colocarPin(zoomCerca) {
